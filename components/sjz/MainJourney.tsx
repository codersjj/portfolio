...
const MainJourney: React.FC<MainJourneyProps> = ({ startAnimation, onVideoStateChange }) => {
    ...
    const handleOpenGift = () => {
        ...
        gsap.to('.gift-lid', { rotation: -45, y: -50, x: 20, opacity: 0, duration: 1, ease: 'power2.inOut' });
        gsap.to('.gift-light', { opacity: 1, scale: 30, duration: 1.5, ease: 'power2.out' });
        gsap.fromTo('.final-message',
            { opacity: 0, scale: 0.5, y: 50 },
            { opacity: 1, scale: 1, y: 0, duration: 1, delay: 0.5, ease: 'back.out(1.2)' }
        );
    };
    ...
    return (
        <div ref={containerRef} className="relative z-10 w-full overflow-hidden">
            ...
            {/* SCENE 1: WELCOME */}
            <section ref={scene1Ref} className="min-h-screen flex flex-col justify-center items-center relative px-6">
                <h1 ref={titleRef} className="text-center">
                    <span className="block text-xl md:text-2xl text-amber-200/80 mb-4 font-light tracking-[0.2em] uppercase">Happy Birthday</span>
            ...
            <section ref={scene2Ref} className="min-h-screen flex flex-col justify-center items-center py-20 px-4 bg-gradient-to-b from-transparent to-slate-900/50">
            ...
            <section className="min-h-screen flex flex-col justify-center items-center py-10 px-4 relative">
                ...
                <div ref={wishSectionRef} className="relative z-10 text-center pb-36">
                    <h2 className="text-3xl md:text-5xl font-serif text-slate-200 mb-8">
                        ...
                        <span className="wish-word inline-block mx-2 text-amber-400">岁岁</span>
                    ...
                    <h2 className="text-2xl md:text-4xl font-serif text-slate-300 mb-8">
                        ...
                        <span className="wish-word inline-block mx-2 text-amber-400">皆胜意</span>
                    </h2>
                    <div className="flex flex-wrap justify-center gap-3 md:gap-4 max-w-2xl mt-8">
                        {['健康', '快乐', '自由', '暴富', '美丽', '平安', '智慧', '从容'].map((word, i) => (
                            <span key={i} className="wish-word inline-block px-4 py-2 bg-white/5 rounded-full border border-white/10 text-amber-200/80 text-sm md:text-base backdrop-blur-sm">
                                {word}
                            </span>
                        ))}
                    </div>
                ...
                <div ref={videoSectionRef} className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl px-4 relative z-10">
                    {/* Video Card 1 */}
                    <div className="video-card group relative aspect-video bg-black rounded-xl overflow-hidden shadow-2xl border border-amber-500/20 cursor-pointer"
                        onClick={() => playVideo('wishes')}>
                        <img src={APP_CONSTANTS.IMAGES.VIDEO_THUMB_1} alt="Blessings" className="w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity duration-500 group-hover:scale-105 transform" />
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                            ...
                            <span className="text-amber-100 font-serif tracking-widest text-lg">老师与朋友的祝福</span>
                        </div>
                    </div>
                    ...
                    <div className="video-card group relative aspect-video bg-black rounded-xl overflow-hidden shadow-2xl border border-amber-500/20 cursor-pointer"
                        ...
                        <img src={APP_CONSTANTS.IMAGES.VIDEO_THUMB_2} alt="Journey" className="w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity duration-500 group-hover:scale-105 transform" />
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <div className="w-16 h-16 rounded-full bg-amber-500/20 backdrop-blur-sm flex items-center justify-center border border-amber-400/50 group-hover:bg-amber-500/40 transition-all mb-4">
                                <Play className="w-6 h-6 text-amber-100 fill-current ml-1" />
                            </div>
                            <span className="text-amber-100 font-serif tracking-widest text-lg">学习之旅记录</span>
                        </div>
                    </div>
                </div>
            </section>
            ...
            {/* SCENE 4: FINAL GIFT */}
            <section ref={scene4Ref} className="scene-5 min-h-screen flex flex-col justify-center items-center relative px-4">
                <div ref={giftRef} className="relative flex flex-col items-center">
                    ...
                    {!giftOpened ? (
                        ...
                        >
                            ...
                            <p className="mt-8 text-amber-200 animate-pulse text-center font-serif tracking-widest">点击打开礼物</p>
                        </div>
                    ) : (
                        <div className="final-message relative z-20 text-center p-8 bg-slate-800/90 backdrop-blur-xl rounded-2xl border border-amber-400/50 shadow-[0_0_50px_rgba(251,191,36,0.2)] max-w-lg mx-auto">
                            ...
                            <div className="text-base md:text-lg text-slate-200 leading-8 font-light space-y-4">
                                <p>张金爽，感谢你来到这个世界。</p>
                                <p>愿未来的日子，<br />不仅有诗和远方，更有触手可及的温暖。</p>
                                <p>这只是一个小小的网页，<br />但装满了我大大的祝福。</p>
                            </div>
